...
ipip.l <- ipip %>% 
  gather("A_1":"O_10", key=key,value=value)
...
ipip.l <- ipip.l %>% 
  separate(key, into = c("trait", "item"), sep="_")
...
exer.avg <- ipip.comp %>% 
  group_by(trait,exer) %>%
  summarise(avg=mean(Average), sem=sd(Average)/sqrt(length(unique(ipip.comp$RID))-1))
